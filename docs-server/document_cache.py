...
class DocumentCache:
    ...
    def check_for_updates(self) -> List[str]:
        ...
        Returns:
            List of document IDs that need updating
        """
        stale_docs = []
        ...
        for document_id, doc in self._documents.items():
            try:
                ...
                if file_path.exists():
                    ...
                    if current_mtime > doc.file_modified:
                        stale_docs.append(document_id)
                else:
                    # File was deleted
                    stale_docs.append(document_id)
            except Exception as e:
                logger.warning(f"Error checking file {doc.path}: {e}")
                stale_docs.append(document_id)
        
        return stale_docs
    
    def refresh_document(self, document_id: str) -> bool:
        """
        Refresh a single document from disk
        ...
        try:
            old_doc = self._documents[document_id]
            file_path = Path(old_doc.path)
            
            if not file_path.exists():
                # File was deleted, remove from cache
                del self._documents[document_id]
                self._total_size -= old_doc.size
                logger.info(f"Removed deleted document from cache: {document_id}")
                return True
            ...
            # Update cache
            self._total_size = self._total_size - old_doc.size + updated_doc.size
            self._documents[document_id] = updated_doc
            
            logger.info(f"Refreshed document in cache: {document_id}")
            return True
    ...
    def list_documents_matching(self, query_regex: str, top_k: int = 100) -> List[str]:
        """
        List documents with matching id.
        
        Parameters:
        - query_regex: Search query string (regex pattern)
        ...
        try:
            ...
            # Search through all document IDs
            matching_ids = []
            for document_id in self._documents.keys():
            ...
            logger.debug(f"Found {len(matching_ids)} documents matching regex '{query_regex}'")
            return matching_ids
            
        except re.error as e:
            logger.error(f"Invalid regex pattern '{query_regex}': {e}")
            return []
